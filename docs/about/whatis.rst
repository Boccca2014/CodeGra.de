What is CodeGrade?
====================
.. include:: codegradefeatures.rst

This is all combined in an online environment that seamlessly integrates in
modern a LMS (e.g. `Canvas <https://www.canvaslms.com/>`_) using the LTI standard. Making
...
----------------------
The CodeGrade Filesystem (or CodeGra.fs) can be used to locally mount a
CodeGrade instance on your computer to browse assignments and files on the
server. The filesystem can also be used by students to locally work on the
...
^^^^^^^^^^^^^^^^^^^^^^^^^^
Accompanying the CodeGrade filesystem are the editor plugins. These plugins
